			...
			if (_GENERATIONS.length === 0) {
				...
				for (let e = 0, el = Math.round(0.2 * s_population); e < el; e++) {
					...
					if (population.length < s_population) {
						...
						// TODO: Push an identical clone to the population array
						// - current[e] is an Agent instance
						// - Agent.prototype.clone() is already implemented
				...
				for (let m = 0, ml = Math.round(0.2 * s_population); m < ml; m++) {
					...
					if (population.length < s_population) {
						...
						// TODO: Push a new randomized agent to population array
						// - agent's brain has randomized weights by default
				...
				while (population.length < s_population) {
					...
					if (population.length < s_population) {
						...
						// TODO: crossover agents and push babies to array
						// - allow s_population to be uneven (e.g. 51)
						// - allow all dominant agents to produce multiple babies
