			...
			for (let p = 0, pl = planes.length; p < pl; p++) {
				...
				if (plane.alive === true) {
					...
					if (agent !== null) {
						...
						// TODO: Brain Computation
						// - First Sensor (input) is relative Plane Position (plane.y / this.height)
						// - Second Sensor (input) is relative Goal Position (next_goal)
						// - If Control (output) is > 0.5, let plane jump.
						// - Agent.prototype.compute(inputs) is already implemented
						// - Plane.prototype.jump is already implemented
